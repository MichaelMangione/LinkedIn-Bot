...
function logEmailCodeHelp() {
  ...
  console.warn(
    "  → Option A: set LINKEDIN_VERIFICATION_CODE=###### in .env or Railway variables, then restart."
  );
  console.warn(
...
async function trySubmitEmailVerificationCode(page) {
  ...
  if (!submitted) {
    ...
    console.log("  → Verification submit via Enter key");
  }

  // Wait for LinkedIn to navigate away from the verification page.
  await page
    .waitForNavigation({ waitUntil: "domcontentloaded", timeout: 10_000 })
    .catch(() => null);
  await sleep(2000);

  const stillOnVerification = await pageNeedsEmailVerification(page);
  if (stillOnVerification) {
    console.warn("  → Verification code rejected or expired — page unchanged after submit.");
